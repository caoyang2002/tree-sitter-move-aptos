...
visibility_modifier: ($) => 
  seq(
    "public", 
    optional(seq("(", choice("package", "friend"), ")"))
  ),
...
function_modifiers: ($) =>
  choice(
    ...
    // 三个修饰符的组合
    seq($.visibility_modifier, $.implementation_modifier, $.function_behavior_modifier)
  ),
...
ability: ($) => choice("copy", "drop", "store", "key"),
    ...
    spec_body: ($) =>
      seq("{", repeat($.package_use_declaration), repeat($._spec_block_memeber), "}"),
    _spec_block_memeber: ($) =>
      choice(
        $.spec_invariant,
        $._spec_function,
...
// 函数类型参数
    function_type_parameters: ($) => field("lambda_parameter",
    $.lambda_expression),
    ...
    lambda_expression: ($) =>
      ...
      // || 1
      // || { 1 }
      prec.left(field("lambda_parameter",choice(
        // |<参数类型列表>| <返回类型>
        // 参数类型列表，至少有一个参数，返回类型
        seq("|",sepBy1(",", field("param_type", $._type)), "|",field("return_type", $._type), ),//
        // |x|{a+b}
        // ||{let c = a+b; c+d};
        // || fun()
      seq("|", optional(sepBy(",",$._variable_identifier)), "|",field("expression",$._expression)), // 闭包
      ),
    ),),
    ...
    closure: $ => seq(
      '|',
     fiele("args",optional($.arg_list)), // 实参列表
      '|',
      field('body', $._expression),
    ),
